...
class ControlWindow:
    def __init__(self, root, geometryString, name, robot: RobotHttpInterface):
        ...
        self.robotFw.grid(row=1, rowspan=2, column=1, columnspan=1, sticky="WE")

        self.robotL = tk.Button(self.robotWindow, text="\u2B9C", command=self.robot.move_left)
        self.robotL.configure(height=self.h, width=self.w)
        self.robotL.grid(row=3, rowspan=2, column=0, columnspan=1, sticky="WE")
        ...
        self.robotR.grid(row=3, rowspan=2, column=2, columnspan=1, sticky="WE")
        ...
        self.robotBw.grid(row=5, rowspan=2, column=1, columnspan=1, sticky="WE")
        ...
        self.robotStop.grid(row=3, rowspan=2, column=1, columnspan=1, sticky="WE")
        ...
        self.robotTl.grid(row=5, rowspan=2, column=0, columnspan=1, sticky="WE")
        ...
        self.robotTr.grid(row=5, rowspan=2, column=2, columnspan=1, sticky="WE")

        self.robotUp = tk.Button(self.robotWindow, text="\u2303", command=self.robot.move_up)
        self.robotUp.configure(height=self.h, width=self.w)
        self.robotUp.grid(row=1, rowspan=2, column=2, columnspan=1, sticky="WE")
        ...
        self.robotDown.grid(row=1, rowspan=2, column=0, columnspan=1, sticky="WE")
        #### --------------------------- ####
    ...
    def getPosition(self):
        ...
        self.labelPosX['text'] = "x = " + str(round(self.pos[0], 6))
        self.labelPosY['text'] = "y = " + str(round(self.pos[1], 6))
        self.labelPosZ['text'] = "z = " + str(round(self.pos[2], 6))


class ControlWindowGirona(ControlWindow):
    def __init__(self, root, geometryString, name, robot: RobotHttpInterface):
        super().__init__(root, geometryString, name, robot)
        ### Controlar Gripper ###
        self.separator2 = tk.Label(self.robotWindow, text=" ")
        self.separator2.grid(row=7, column=0, columnspan=10)

        self.buttonOpenG = tk.Button(self.robotWindow, text="Open\nGripper", command=self.robot.open_gripper)
        self.buttonOpenG.grid(row=8, column=0, rowspan=2, sticky="NSWE")
        self.buttonCloseG = tk.Button(self.robotWindow, text="Close\nGripper", command=self.robot.close_gripper)
        self.buttonCloseG.grid(row=8, column=1, rowspan=2, sticky="NSWE")
        self.buttonStopG = tk.Button(self.robotWindow, text="Stop\nGripper", command=self.robot.stop_gripper)
        self.buttonStopG.grid(row=8, column=2, rowspan=2, sticky="NSWE")
        ### ----------------- ###
